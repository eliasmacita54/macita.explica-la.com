...
class AssignClassTeacherController extends Controller
{
    ...
    public function insert(Request $request)
    {
        ...
        if (!empty($request->teacher_id))
        {
            ...
            {
                $getAlreadyFirst = AssignClassTeacherModel::getAlreadyFirst($request->classs_id, $teacher_id);
            ...
            return redirect('admin/assign_class_teacher/list')->with('success', 'Assign Class Teacher with success');
        }
        else
        {
    ...
    public function update(Request $request, $id){
        ...
        if (!empty($request->teacher_id)){
           ...
           // return redirect('admin/assign_class_teacher/list')->with('success', 'assign with success');

           return redirect('admin/assign_class_teacher/list')->with('success', 'Edit Assign Class Teacher with success');
    }
}
public function edit_single($id)
    {
        $getRecord = AssignClassTeacherModel::getSingle($id);
            ...
            if (!empty($getRecord)) {
                ...
                $data['getClass'] = ClassModel::getClass();
                $data['getTeacher'] = User::getTeacherClass();
                $data['header_title'] = "edit_single Assign Class Teacher";
        ...
        public function update_single($id, Request $request)
        {
                $getAlreadyFirst = AssignClassTeacherModel::getAlreadyFirst($request->class_id, $request->teacher_id);
                        if(!empty($getAlreadyFirst))
                        {
                            $getAlreadyFirst->status = $request->status;
                            $getAlreadyFirst->save();
                            return redirect('admin/assign_class_teacher/list')->with('success', 'status successufuly changed');
                        }
                        else
                        {
    ...
    public function delete($id)
    {
        ...
        $save->is_delete = 1;
        $save->save();
        return redirect()->back()->with('success', 'Deleted successuflly');
