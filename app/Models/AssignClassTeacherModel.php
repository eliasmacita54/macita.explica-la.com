...
class AssignClassTeacherModel extends Model
{
   ...
   static public function getRecord()
   {
       ...
       if(!empty(Request::get('status')))
       {
        ...
        $return->where('assign_class_teacher.status', '=', $status);
       }

       if(!empty(Request::get('date')))
       {
        $return->where('assign_class_teacher.created_at', '=',Request::get('date'));
       }
...
{
    return self::where('class_id', '=', $class_id)->where('is_delete','=', 1)->get();
}
...
{
   $value = WeekModel::getWeekUsingName(date('1'));
   return ClassSubjectTimetableModel::getRecordClassSubject($class_id, $subject_id, $value->id);
  }
